...
2. **Plant the failures — don't wait for them.** Seed the failure mode into the data or queries so every student hits it by design:
   ...
   - sensitive values buried in data that must be detected and handled
...
4. **Tie to a production gate.** Each challenge should move the system past one gate: regression evals on every change, cost/token observability, scoped access, tracing/logging, or correct handling of a planted failure. Name the gate in the doc.
...
- `student` — what students use. Gets: the same spec material. NEVER copy solutions from `instructor`; the skeleton files students must fill in themselves are `lib/agents/*`, `lib/graph.ts`, `app/api/chat/route.ts`, `app/api/chat-graph/route.ts`, `lib/calendar.ts`, `lib/scheduling.ts`, `lib/evals/*`, `lib/security/*`.
...
2. Write stubs that **compile and fail on assertions, not imports**: functions `throw new Error('Not implemented')`, routes return 501. Include TODO comments stating requirements, not steps (match `docs/CHALLENGE-NOTE-INGEST.md` tone).
