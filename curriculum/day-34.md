# Day 34 — Adversarial Day: The Poisoned Document

**Needs: the running system; `docs/CHALLENGE-POISONED-DOCS.md` and `data/security/poisoned/`**

## Today you will

- Pay off a promise made weeks ago: prompt injection arriving through a door nobody guards
...
On the failure day, you met prompt injection through the front door — a user typing "ignore your instructions." You tested it, hardened the prompt, and the system mostly held. Then this course promised the same attack would return "through a much sneakier entrance." Here it is.

The entrance is **the data itself**. Your system retrieves documents and pastes them into the LLM's context. What if a document *contains instructions*? A clinical note that reads:
...
When retrieval surfaces that note, your pipeline concatenates it into the context — and the LLM cannot tell *your* instructions from text *inside the data*. To the model, it is all just tokens in the window. This is **indirect prompt injection**, and it is categorically nastier than the front-door kind for three reasons:
...
3. **It is invisible in normal use.** The note looks like a note. Nothing trips until the day it is retrieved for the wrong query.
...
Your upload API from three days ago is one way that note gets in. Your MCP server is another. Every ingestion path you built is also an attack surface — which is the day's uncomfortable symmetry: the gates that let data *in* are the gates an attacker walks *through*.
...
Run the demo, ingest a poisoned note, ask an innocent question that retrieves it, and watch your system obey the attacker. Do this *first*. A defense you build before you have felt the attack is a defense against your imagination; a defense you build after is a defense against the real thing.

### 2. Defense in depth — three layers, because one is not enough
...
- **Sandboxing** (`buildSandboxedContext`) — the structural fix: wrap retrieved content in unmistakable delimiters and tell the model, in the system prompt, that everything inside is *data to analyze, never instructions to follow*. This is the load-bearing layer — it does not try to *catch* the attack, it removes the ambiguity the attack depends on.
...
- **Defending the answerer but not the tools.** If a poisoned note can trick the LLM into *calling a tool* (the fake-invocation doc), the damage is no longer just a wrong answer — it is unauthorized action. Sandboxing has to cover the tool-use path, not just the prose answer.
- **Believing you finished.** You will not catch every attack today, and the challenge will not claim you did. Defense-in-depth is about raising cost and shrinking blast radius, not achieving immunity. "Done" is the wrong frame; "the next attack is now more expensive and more contained" is the right one.

## Your turn

The challenge is the your-turn. Additionally, capture for your capstone:
...
**Why the prompt rule misses it:** that rule conditions the model to be skeptical of the *user turn* — the slot where front-door injection arrives. Indirect injection arrives in the *context* slot, wearing the costume of retrieved data the model was *told to trust and use*. You cannot tell the model "trust the retrieved context" and "distrust the retrieved context" in the same breath; the poisoned note exploits exactly that contradiction. The front-door defense and the back-door attack pass each other without touching, which is why a *structural* fix (sandboxing — explicitly marking the trust level of each region of the context) beats any *content* rule.

**The one layer to keep: sandboxing.** Detection and sanitization are pattern-based, and pattern-based defenses are in a losing arms race — every blocklist is one rephrase behind. Sandboxing attacks the root: it removes the ambiguity between instruction and data that *all* injection depends on, by structurally separating "here are your orders" from "here is material to analyze." It is not perfect (a sufficiently clever payload can still try to break out of the delimiters), but it is the only layer that defends against attacks you have not seen yet — and the attacks you have not seen yet are the ones that get you. The general principle, true far beyond this day: prefer defenses that remove a capability over defenses that enumerate bad inputs.

**The honest gap** most students land on: a payload that contains no recognizable injection *patterns* and respects the sandbox delimiters while still subtly biasing the answer (e.g., a note that is technically data but emotionally loaded, nudging the model's summary). Naming that gap is not a failure of your work — it is the mark of an engineer who has measured their defenses instead of assuming them, which is the whole posture this course has been building toward, and exactly what the capstone postmortem asks you to demonstrate.

</details>

## Further reading (optional)

- [OWASP: LLM01 prompt injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/) — reread the "indirect" section now that you have been hit by it; it will read like a description of your afternoon
