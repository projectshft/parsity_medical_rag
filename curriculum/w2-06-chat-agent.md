...
- "What do the notes say about sleep problems for patients with depression?" → hybrid: notes scoped to depression patients.
- a follow-up that depends on the previous turn — watch the conversation history flowing through `runAgent`.
...
- **Touching the scheduling wiring.** It belongs to the human-in-the-loop week. Leave `detectSchedulingIntent` and the scheduling card alone today.
...
**On "what's a normal A1C?":** two defensible behaviors — refuse-and-redirect, or answer the general question *clearly labeled* as general knowledge, separate from patient data. What's indefensible is answering it in a way that's indistinguishable from a records-grounded claim. Whichever you choose, your prompt now encodes a *policy decision* — notice that you, the engineer, just made one. Production prompts are full of them; good systems make them on purpose.
