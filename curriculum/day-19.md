# Day 19 — Structured Outputs: Making LLMs Return Data, Not Prose

**Needs: `OPENAI_API_KEY`; pennies of usage**

## Today you will

- Make an LLM return a *typed object* your code can branch on — reliably, every time
- Learn the schema-first pattern this repo uses for every LLM-as-component call
- Build your own extractor from scratch

## Concept

So far the LLM has played one role: the *answerer* at the end of the pipeline, producing prose for a human. Today it takes on a second, very different role: **a component in the middle of the pipeline**, whose output is consumed by *code*.

Code can't branch on prose. If the model replies:

> "This looks like a fairly urgent billing issue, and the customer seems quite upset…"
...
The naive approach — adding *"please respond in JSON"* to the prompt — fails in every way that matters: sometimes you get markdown fences around the JSON, sometimes a chatty preamble, sometimes a field goes missing, sometimes `"urgency": "very high"` appears even though you never offered that value. Works in the demo, breaks in production, which is the worst possible combination.
...
- **`.describe()` is prompting.** Every field description is read by the model — `urgency: z.enum(['low','medium','high']).describe('How urgent the issue is')` is simultaneously a type constraint *and* an instruction. Schema-first design moves half your prompt into the place where it can't drift out of sync with the types.
...
A warm-up extractor, no medicine involved — customer support email triage. Scratch script:
...
Look at what you got for free: the enum held (`high`, not `very urgent!!`), the account number was found *and typed* (`nullable` meant "null is a legal answer, don't invent one"), and `ticket` is a fully typed object — your editor autocompletes `ticket.urgency` and the compiler knows its three possible values.

Now break it on purpose — this is the instructive part:

1. Remove `temperature: 0`, run five times, watch borderline fields (is this `medium` or `high`?) wobble between runs. Determinism matters for components.
...
- **Undescribed fields.** `urgency: z.enum([...])` with no `.describe()` makes the model guess your definition of urgency. Descriptions are where you encode the judgment calls ("high = customer threatens to cancel or mentions money lost").
...
**The redundant-looking parse:** `zodTextFormat` constrains generation, but the boundary between "what the API returned" and "what your types claim" deserves a runtime check — SDK versions drift, models occasionally emit edge-case output, and refusals/length-truncations can yield non-conformant results. `Schema.parse` converts all of that into one loud, located error. The pattern is general: **validate at every boundary where types are claimed but not proven.**
...
**A calendar-extractor wrinkle worth noticing:** "next Tuesday" needs a reference date the model doesn't reliably have. Either pass today's date in the prompt, or have the schema demand relative output (`dayOfWeek` + `weeksFromNow`) and resolve it in code. Knowing *which work belongs to the model and which to code* is structured-output design in a nutshell — and it's tomorrow's whole subject.
