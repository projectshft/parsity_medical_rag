# Wiring MCP into Claude Desktop and Cursor

**Needs: the working MCP server; Claude Desktop and/or Cursor installed**
...
You tested the server with pipes. Now a production AI application launches it, discovers your tools, and decides — mid-conversation, on its own judgment — to call them.
...
<!-- TODO: capture Claude Desktop with the medical-rag MCP server connected (tools icon visible) -->
...
Watch what happens before each answer: the assistant announces a tool call, shows the arguments it chose, and waits for approval — the client's built-in human-in-the-loop default. (You'll build your own version of that pattern later in this block.) Approve, and your code runs. The answer that comes back is *their* model reading *your* formatted text.
...
- **Tools hang** → something wrote to stdout (the previous lesson's warning, now live-fire). Find the stray `console.log`.
