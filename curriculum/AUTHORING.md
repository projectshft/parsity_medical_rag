...
- 2026-06-27 · Sparse+dense hybrid **stays optional research** in THIS (medical-RAG) curriculum — Day 16's "Going further (optional)" section is unchanged, because here the lexical/exact-match half is owned by Postgres, so sparse vectors aren't needed. Key clarification: the "AI Engineering Certificate Program" Form B proposal (reference copy saved to `docs/AI-Engineering-Certificate-Program.md`) actually describes a **separate repo, `mini_rag`** (modules: fine-tuning, LinkedIn agent, text-to-SQL, `9-rag-agent` hybrid) — NOT this medical-RAG course. The runnable sparse+dense experiment lives there (`mini_rag` `app/scripts/exercises/hybrid-search-demo.ts`, now with a custom-query mode + `exercise:hybrid` npm alias). So the proposal's divergences from medical-RAG are expected — different course — and nothing in medical-RAG needs to chase them.
