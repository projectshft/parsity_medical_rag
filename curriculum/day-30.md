# Day 30 — Build Day: A New Tool, with an Audit Trail

**Needs: the secured MCP server; the traced pipeline**

## Today you will

- Design and ship a fourth MCP tool, end to end: spec → scopes → implementation → audit → proof
...
This is a **build day**. The training wheels from the MCP days are off: you make every decision yourself — what the tool does, what it requires, what it logs, and how you *prove* all three.
...
The provided `mcp-server/audit.ts` has the machinery: structured entries, sensitive-field redaction, a `withAudit` wrapper, even `withAuthAndAudit` composing both layers. Machinery isn't a trail — the design is choosing entry contents so those four questions have answers.

## Implementation

### 1. Pick your tool

Use the fourth-tool spec you wrote on the client-wiring day if you have one. Otherwise, candidates in rough order of ambition:

- `list_conditions` — distinct conditions with patient counts (analytics; `read` scope)
- `compare_lab_across_patients` — one lab, several patients, values side by side (the brute-force gap many found on the wiring day; `read` or `read_pii`? — *a real decision: are lab values attached to patient ids PII?*)
...
2. **Scope decision, written down**: which scopes and one sentence why — then added to `TOOL_SCOPES`
3. **Implementation** behind the same auth + audit pattern as the existing three — if you built `withAuthAndAudit` during the challenge, this is one wrapper call; *feel* the payoff of structural security
...
2. A correctly-scoped key calls it successfully → audit entry exists with key identity, params (redacted where sensitive), and result summary
3. An *under*-scoped key is denied cleanly → the denial is readable by the calling model, **and the denial is in the log**
4. A malformed call (bad param types) errors without crashing the server
5. The log file answers question #1 ("who touched patient X today?") with a one-liner — `grep`/`jq` is fine; the point is the entry *contains* the answer

### 4. Close the loop on cost

One question, answered with the trace data you now have: **what does one call to your new tool cost?** Count the LLM calls in its trace (embedding calls count), estimate tokens in/out for each, and write the per-call figure in your notes. If you built `summarize_patient`, compare its cost to `list_conditions` — they can differ by 100×. A tool server where every tool looks free until the invoice arrives is the norm in this industry; yours now has per-tool price tags, which puts you ahead of most production teams.
...
- **Audit entries that summarize too aggressively.** `"called compare_lab"` can't answer "who touched patient X" — the patient ids belong in the entry. But the *lab values* might not (that's serving PII into a log file). Redaction is a per-field decision, and the provided `redactSensitiveFields` list is a starting point, not an exemption from thinking.
...
- **Defend the design:** Your fourth tool — why this tool, why these scopes, what its audit entry contains and what it deliberately omits, and what one call costs. Name the alternative scope decision you rejected.
- **Teach back:** Explain to a non-engineer why "the AI can book appointments" required a confirmation card, but "the AI can search records" didn't — and how the same logic decides which future features need a human gate.

**Submit:** [Typeform — submission](https://form.typeform.com/to/PLACEHOLDER-DAY30) <!-- PLACEHOLDER: replace with real Typeform URL -->

## Rest day next

Count what this block shipped: a protocol server any AI client can use, key-based identity, scoped authorization, an audit trail that answers investigator questions, request-level tracing with per-step timings, and a human gate on the one action that touches the world. **Tomorrow is a rest day.** The final block remains: the production gates — where the system gets locked down for *people* (not just API keys), attacked through its data, and judged, end to end, by the standard you've been building all along: no metric, no decision.
