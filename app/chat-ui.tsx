...
// One UI, two orchestrators behind it: `/` posts to /api/chat (we hard-code the
// pipeline), `/tools` posts to /api/chat-tools (the model picks the tools).
export default function ChatUI({
  ...
  const parseSchedulingAction = (content: string): { text: string; action?: SchedulingAction } => {
    ...
    // Hide a half-streamed marker so it doesn't flicker into the bubble.
  ...
  const sendQuery = async (userMessage: string) => {
    ...
    try {
      ...
      // Scheduling action arrives either in a response header (/api/chat sets it
      // on the needsAppt short-circuit) or as a trailing marker in the stream
      // (/api/chat-tools, where the tool fires after headers are sent).
      const schedHeader = response.headers.get("X-Scheduling-Action");
      const action = schedHeader
        ? (JSON.parse(decodeURIComponent(schedHeader)) as SchedulingAction)
  ...
  return (
    <div className="flex flex-col h-screen bg-copilot-bg">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-copilot-border bg-copilot-sidebar">
        ...
        <span className="px-3 py-1.5 text-xs rounded-lg border border-copilot-border text-copilot-muted font-mono">
          {badge} · {endpoint}
        </span>
      </header>
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            ...
            {isStreaming &&
              (messages[messages.length - 1]?.role !== "assistant" ||
                ...
                <div className="flex justify-start">
                  <div className="bg-copilot-sidebar rounded-lg px-4 py-3 flex items-center gap-2">
                    ...
                    <span className="text-sm text-copilot-muted">Thinking…</span>
                  </div>
                </div>
              )}
