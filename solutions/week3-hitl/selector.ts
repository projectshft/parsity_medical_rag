...
const planAgentSchema = z.object({
	...
	needsAppt: z
		.boolean()
		.describe(
			'True if the user wants to BOOK/SCHEDULE an appointment (an action), not just look up data. Routing only — do NOT extract the name/date here.',
		),
	reason: z
		.string()
		.describe(
...
export type Plan = {
	...
	needsSearch: boolean;
	/** true = a booking request — skip SQL/RAG, hand to the scheduling agent. */
	needsAppt: boolean;
...
export async function select(
	...
	const answer = await openai.responses.parse({
        ...
        Our vector store has notes about patients. Here is an example
          ...
          If the user wants to BOOK or SCHEDULE an appointment (e.g. "schedule Avery for Tuesday"), set needsAppt=true — that's an action, not a lookup, so useSql and useRag should be false.
