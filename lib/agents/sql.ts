 ...
 * query builders — the model writes the WHERE / ORDER BY / LIMIT itself. This is
 * what kills the whole class of query-layer bugs — "smokers under 50", "youngest
 * diabetic", superlatives, combined filters.
 *
 * Two things are the lesson:
 *   1. SAFETY — an LLM writing SQL is an injection surface. `assertReadOnly`
 *      accepts ONLY a single read-only SELECT (no INSERT/UPDATE/DELETE/DDL/`;`)
 *      and we enforce a LIMIT. In production, ALSO point DATABASE_URL at a
...
/** LLM writes one read-only SELECT; we validate it, run it, return the rows. */
